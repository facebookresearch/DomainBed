            ...
            for hparams_key, hparams_value in hparams_dict.items():
                ...
                for hparam in hparams_value:
                    trial_averages = (grouped_records
                        .filter_equals(f"algorithm, dataset,{hparams_key}", (algorithm, dataset,hparam))
                        .group("trial_seed")
                        .map(lambda trial_seed, group:
                            group.select("sweep_acc").mean()
                        )
                    )
                    mean, err = format_mean(trial_averages)
                    value_mean_err.append((hparam, mean, err))
                dict = {"algorithm": algorithm, "dataset": dataset, "hparams_key": hparams_key, "value_mean_err": value_mean_err}
